...
public class LinkedPath<E> implements Path<E> {
    ...
    public boolean appendEnd(Edge<E1> edge) {
        Vertex<E>[] endVertices = (Vertex<E>[]) edge.endVertices();
        if (edges.contains(edge))
            return false; // edge is already used in the path
        if (endVertex.equals(endVertices[0])) {
            edges.add((Edge<E>) edge);
    ...
    public boolean containsEdge(Edge<E1> edge) {
    ...
    @Override
    public <E1> boolean containsEdge(Edge<E1> incidentEdge) {
        return false;
    }

    @Override
    public <E1> void appendEnd(Edge<E1> incidentEdge) {
